...
#define LOCTEXT_NAMESPACE "SScreenLog"
/** Expression context to test the given messages against the current text filter */
class FGameLogFilter_TextFilterExpressionContext : public ITextFilterExpressionContext
{
public:
	explicit FGameLogFilter_TextFilterExpressionContext(const FGameLogMessage& InMessage) : Message(&InMessage) {}
...
private:
	...
	const FGameLogMessage* Message;
};

SGameConsoleInputBox::SGameConsoleInputBox()
	: bIgnoreUIUpdate(false)
	, bHasTicked(false)
	, ConsoleBorderBrus(TEXT(""), FMargin(), FSlateColor(FLinearColor(0.5f, 0.5f, 0.5f, 0.2f)), ESlateBrushImageType::Linear)
{
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SGameConsoleInputBox::Construct(const FArguments& InArgs)
{
	...
	IModularFeatures::Get().OnModularFeatureRegistered().AddSP(this, &SGameConsoleInputBox::OnCommandExecutorRegistered);
	IModularFeatures::Get().OnModularFeatureUnregistered().AddSP(this, &SGameConsoleInputBox::OnCommandExecutorUnregistered);
	EPopupMethod PopupMethod = GIsEditor ? EPopupMethod::CreateNewWindow : EPopupMethod::UseCurrentWindow;

	FTextBlockStyle filterTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");
	filterTextStyle.SetFontSize(30);

	ETBStyle.Font.Size = 30;

	ConsoleBorderBrus = FSlateBoxBrush(NAME_None, FVector2D(), FMargin(), FLinearColor(1, 1, 1, 1));
	ChildSlot
	[
		...
		[
			...
			[
				SNew(SComboButton)
				.IsEnabled(this, &SGameConsoleInputBox::IsCommandExecutorMenuEnabled)
				//.ComboButtonStyle(FEditorStyle::Get(), "GenericFilters.ComboButtonStyle")
				.ForegroundColor(FLinearColor::White)
				.ContentPadding(0)
				.OnGetMenuContent(this, &SGameConsoleInputBox::GetCommandExecutorMenuContent)
				.ButtonContent()
				[
					SNew(STextBlock)
					.TextStyle(&filterTextStyle)
					.Text(this, &SGameConsoleInputBox::GetActiveCommandExecutorDisplayName)
				]
			]
			...
			[
				...
				.Style(&ETBStyle)
				.HintText(this, &SGameConsoleInputBox::GetActiveCommandExecutorHintText)
				.AllowMultiLine(this, &SGameConsoleInputBox::GetActiveCommandExecutorAllowMultiLine)
				.OnTextCommitted(this, &SGameConsoleInputBox::OnTextCommitted)
				.OnTextChanged(this, &SGameConsoleInputBox::OnTextChanged)
				.OnKeyCharHandler(this, &SGameConsoleInputBox::OnKeyCharHandler)
				.OnKeyDownHandler(this, &SGameConsoleInputBox::OnKeyDownHandler)
		...
		(
			...
			[
				...
				[
					...
					.OnGenerateRow(this, &SGameConsoleInputBox::MakeSuggestionListItemWidget)
					.OnSelectionChanged(this, &SGameConsoleInputBox::SuggestionSelectionChanged)
					.ItemHeight(18)
				]
			]
		)
...
void SGameConsoleInputBox::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	bHasTicked = true;
...
void SGameConsoleInputBox::SuggestionSelectionChanged(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	if (bIgnoreUIUpdate)
	{
		return;
...
FOptionalSize SGameConsoleInputBox::GetSelectionListMaxWidth() const
{
...
TSharedRef<ITableRow> SGameConsoleInputBox::MakeSuggestionListItemWidget(TSharedPtr<FString> Text, const TSharedRef<STableViewBase>& OwnerTable)
{
	check(Text.IsValid());
...
void SGameConsoleInputBox::OnTextChanged(const FText& InText)
{
	if (bIgnoreUIUpdate)
	{
		return;
...
void SGameConsoleInputBox::OnTextCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
...
FReply SGameConsoleInputBox::OnPreviewKeyDown(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
...
void SGameConsoleInputBox::SetSuggestions(TArray<FString>& Elements, FText Highlight)
{
...
void SGameConsoleInputBox::OnFocusLost(const FFocusEvent& InFocusEvent)
{
	//	SuggestionBox->SetIsOpen(false);
}

void SGameConsoleInputBox::MarkActiveSuggestion()
{
	bIgnoreUIUpdate = true;
	if (Suggestions.HasSelectedSuggestion())
	{
...
void SGameConsoleInputBox::ClearSuggestions()
{
	SuggestionBox->SetIsOpen(false);
	Suggestions.Reset();
}

void SGameConsoleInputBox::OnCommandExecutorRegistered(const FName& Type, class IModularFeature* ModularFeature)
{
...
void SGameConsoleInputBox::OnCommandExecutorUnregistered(const FName& Type, class IModularFeature* ModularFeature)
{
...
void SGameConsoleInputBox::SyncActiveCommandExecutor()
{
...
void SGameConsoleInputBox::SetActiveCommandExecutor(const FName InExecName)
{
...
FText SGameConsoleInputBox::GetActiveCommandExecutorDisplayName() const
{
	if (ActiveCommandExecutor)
	{
...
FText SGameConsoleInputBox::GetActiveCommandExecutorHintText() const
{
	if (ActiveCommandExecutor)
	{
...
bool SGameConsoleInputBox::GetActiveCommandExecutorAllowMultiLine() const
{
	if (ActiveCommandExecutor)
	{
...
bool SGameConsoleInputBox::IsCommandExecutorMenuEnabled() const
{
...
TSharedRef<SWidget> SGameConsoleInputBox::GetCommandExecutorMenuContent()
{
	...
	MenuBuilder.BeginSection("CmdExecEntries");
	{
		...
		for (const IConsoleCommandExecutor* CommandExecutor : CommandExecutors)
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP(this, &SGameConsoleInputBox::SetActiveCommandExecutor, CommandExecutor->GetName()),
...
FReply SGameConsoleInputBox::OnKeyDownHandler(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply SGameConsoleInputBox::OnKeyCharHandler(const FGeometry& MyGeometry, const FCharacterEvent& InCharacterEvent)
{
...
TSharedRef< FScreenLogTextLayoutMarshaller > FScreenLogTextLayoutMarshaller::Create(TArray< TSharedPtr<FGameLogMessage> > InMessages, FGameLogFilter* InFilter)
{
	return MakeShareable(new FScreenLogTextLayoutMarshaller(MoveTemp(InMessages), InFilter));
}

FScreenLogTextLayoutMarshaller::~FScreenLogTextLayoutMarshaller()
{
}

void FScreenLogTextLayoutMarshaller::SetText(const FString& SourceString, FTextLayout& TargetTextLayout)
{
	TextLayout = &TargetTextLayout;
	AppendMessagesToTextLayout(Messages);
}

void FScreenLogTextLayoutMarshaller::GetText(FString& TargetString, const FTextLayout& SourceTextLayout)
{
	SourceTextLayout.GetAsText(TargetString);
}

bool FScreenLogTextLayoutMarshaller::AppendMessage(const TCHAR* InText, const ELogVerbosity::Type InVerbosity, const FName& InCategory)
{
	TArray< TSharedPtr<FGameLogMessage> > NewMessages;
	if (SScreenLog::CreateLogMessages(InText, InVerbosity, InCategory, NewMessages))
	{
...
void FScreenLogTextLayoutMarshaller::AppendMessageToTextLayout(const TSharedPtr<FGameLogMessage>& InMessage)
{
	...
	FTextBlockStyle MessageTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText"); //= FEditorStyle::Get().GetWidgetStyle<FTextBlockStyle>(InMessage->Style);

	FLinearColor lc(0.f, 1.0, 0.3f);
	switch (InMessage->Verbosity)
	{
	case ELogVerbosity::Warning:
		lc = FLinearColor(1.f, 1.f, 0.0f);
		break;
	case ELogVerbosity::Error:
		lc = FLinearColor(1.f, 0.f, 0.0f);
		break;
	default:
		break;
	}
	MessageTextStyle.SetColorAndOpacity(FSlateColor(lc));
	MessageTextStyle.SetFontSize(20);
...
void FScreenLogTextLayoutMarshaller::AppendMessagesToTextLayout(const TArray<TSharedPtr<FGameLogMessage>>& InMessages)
{
	...
	for (const auto& CurrentMessage : InMessages)
	{
		...
		switch (CurrentMessage->Verbosity)
		{
		case ELogVerbosity::Warning:
			lc = FLinearColor(1.f, 1.f, 0.0f);
			break;
		...
		FTextBlockStyle MessageTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");// = FEditorStyle::Get().GetWidgetStyle<FTextBlockStyle>(CurrentMessage->Style);
		MessageTextStyle.SetColorAndOpacity(FSlateColor(lc));
		MessageTextStyle.SetFontSize(20);
...
void FScreenLogTextLayoutMarshaller::ClearMessages()
{
	Messages.Empty();
	MakeDirty();
}

void FScreenLogTextLayoutMarshaller::CountMessages()
{
...
int32 FScreenLogTextLayoutMarshaller::GetNumMessages() const
{
	return Messages.Num();
}

int32 FScreenLogTextLayoutMarshaller::GetNumFilteredMessages()
{
...
void FScreenLogTextLayoutMarshaller::MarkMessagesCacheAsDirty()
{
	bNumMessagesCacheDirty = true;
}

FScreenLogTextLayoutMarshaller::FScreenLogTextLayoutMarshaller(TArray< TSharedPtr<FGameLogMessage> > InMessages, FGameLogFilter* InFilter)
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SScreenLog::Construct(const FArguments& InArgs)
{
	...
	MessagesTextMarshaller = FScreenLogTextLayoutMarshaller::Create(InArgs._Messages, &Filter);

	ETBStyle = FCoreStyle::Get().GetWidgetStyle<FEditableTextBoxStyle>("NormalEditableTextBox");
	ETBStyle.ScrollBarStyle.VerticalTopSlotImage.ImageSize		= FVector2D(30.0f, 30.0f);
	ETBStyle.ScrollBarStyle.HorizontalTopSlotImage.ImageSize	= FVector2D(30.0f, 30.0f);
	ETBStyle.ScrollBarStyle.VerticalBottomSlotImage.ImageSize	= FVector2D(30.0f, 30.0f);
	ETBStyle.ScrollBarStyle.HorizontalBottomSlotImage.ImageSize = FVector2D(30.0f, 30.0f);

	MessagesTextBox = SNew(SMultiLineEditableTextBox)
		.Style(&ETBStyle)
		//.TextStyle(FEditorStyle::Get(), "Log.Normal")
		.ForegroundColor(FLinearColor(0.5f, 0.5f, 0.5f,0.5f))
		.BackgroundColor(FLinearColor(0.5f, 0.5f, 0.5f, 0.3f))
		.Marshaller(MessagesTextMarshaller)
		.IsReadOnly(true)
		.AlwaysShowScrollbars(true)
		.OnVScrollBarUserScrolled(this, &SScreenLog::OnUserScrolled)
		.ContextMenuExtender(this, &SScreenLog::ExtendTextBoxMenu);
	
	FTextBlockStyle filterTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");
	filterTextStyle.SetFontSize(20);

	/*FSearchBoxStyle */SearchStyle = FCoreStyle::Get().GetWidgetStyle<FSearchBoxStyle>("SearchBox");
	SearchStyle.TextBoxStyle = FCoreStyle::Get().GetWidgetStyle<FEditableTextBoxStyle>("NormalEditableTextBox");
	SearchStyle.TextBoxStyle.Font.Size = 30;
	SearchStyle.ActiveFontInfo.Size = 30;
	SearchStyle.ClearImage.ImageSize = FVector2D(50.f, 50.f);
	SearchStyle.GlassImage.ImageSize = FVector2D(50.f, 50.f);
		...
		[
			...
			[
				...
				[
					...
					.OnGetMenuContent(this, &SScreenLog::MakeAddFilterMenu)
					.HasDownArrow(true)
				...
				[
					...
					.OnTextChanged(this, &SScreenLog::OnFilterTextChanged)
					.OnTextCommitted(this, &SScreenLog::OnFilterTextCommitted)
			...
			[
				...
				[
					SNew(SGameConsoleInputBox)
					.OnConsoleCommandExecuted(this, &SScreenLog::OnConsoleCommandExecuted)
...
bool SScreenLog::CreateLogMessages(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FGameLogMessage> >& OutMessages)
{
			...
			if (!LineRange.IsEmpty())
			{
				...
				for (int32 CurrentStartIndex = 0; CurrentStartIndex < Line.Len();)
				{
					...
					if (bIsFirstLineInMessage)
					{
						...
						OutMessages.Add(MakeShared<FGameLogMessage>(MakeShared<FString>(MessagePrefix + HardWrapLine), Verbosity, Category, Style));
					}
					else
					{
						...
						OutMessages.Add(MakeShared<FGameLogMessage>(MakeShared<FString>(MoveTemp(HardWrapLine)), Verbosity, Category, Style));
					}
...
void SScreenLog::Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category)
{
...
void SScreenLog::ExtendTextBoxMenu(FMenuBuilder& Builder)
{
	FUIAction ClearOutputLogAction(
		FExecuteAction::CreateRaw(this, &SScreenLog::OnClearLog),
		FCanExecuteAction::CreateSP(this, &SScreenLog::CanClearLog)
	);
...
void SScreenLog::OnUserScrolled(float ScrollOffset)
{
	bIsUserScrolled = ScrollOffset < 1.0 && !FMath::IsNearlyEqual(ScrollOffset, 1.0f);
}

bool SScreenLog::CanClearLog() const
{
	return MessagesTextMarshaller->GetNumMessages() > 0;
}

void SScreenLog::OnConsoleCommandExecuted()
{
	RequestForceScroll();
}

void SScreenLog::RequestForceScroll()
{
...
void SScreenLog::OnFilterTextChanged(const FText& InFilterText)
{
...
void SScreenLog::OnFilterTextCommitted(const FText& InFilterText, ETextCommit::Type InCommitType)
{
	OnFilterTextChanged(InFilterText);
}

TSharedRef<SWidget> SScreenLog::MakeAddFilterMenu()
{
	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr);
	FSlateIcon tIcon;
	//con.GetIcon()->ImageSize = FVector2D(100.f, 100.f);
	FTextBlockStyle filterTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");
	filterTextStyle.SetFontSize(20);
	...
	MenuBuilder.BeginSection("OutputLogVerbosityEntries", LOCTEXT("OutputLogVerbosityHeading", "Verbosity"));
	{
		...
		TSharedRef<SWidget> messagesButton = SNew(STextBlock)
			.TextStyle(&filterTextStyle)
			.Text(LOCTEXT("ShowMessages", "Messages"))
			.ToolTipText(LOCTEXT("ShowMessages_Tooltip", "Filter the Output Log to show messages"));

		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateSP(this, &SScreenLog::VerbosityLogs_Execute),
				FCanExecuteAction::CreateLambda([] { return true; }),
				FIsActionChecked::CreateSP(this, &SScreenLog::VerbosityLogs_IsChecked)),
			messagesButton,
			NAME_None,
		...
		TSharedRef<SWidget> warningButton = SNew(STextBlock)
			.TextStyle(&filterTextStyle)
			.Text(LOCTEXT("ShowWarnings", "Warnings"))
			.ToolTipText(LOCTEXT("ShowWarnings_Tooltip", "Filter the Output Log to show warnings"));

		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateSP(this, &SScreenLog::VerbosityWarnings_Execute),
				FCanExecuteAction::CreateLambda([] { return true; }),
				FIsActionChecked::CreateSP(this, &SScreenLog::VerbosityWarnings_IsChecked)),
			warningButton,
			NAME_None,
		...
		TSharedRef<SWidget> errorsButton = SNew(STextBlock)
			.TextStyle(&filterTextStyle)
			.Text(LOCTEXT("ShowErrors", "Errors"))
			.ToolTipText(LOCTEXT("ShowErrors_Tooltip", "Filter the Output Log to show errors"));

		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateSP(this, &SScreenLog::VerbosityErrors_Execute),
				FCanExecuteAction::CreateLambda([] { return true; }),
				FIsActionChecked::CreateSP(this, &SScreenLog::VerbosityErrors_IsChecked)),
			errorsButton,
			NAME_None,
	...
	{
		TSharedRef<SWidget> categoryButton = SNew(STextBlock)
			.TextStyle(&filterTextStyle)
		...
		MenuBuilder.AddSubMenu(
			categoryButton,
			FNewMenuDelegate::CreateSP(this, &SScreenLog::MakeSelectCategoriesSubMenu)
		);
	}
	//MenuBuilder.MakeWidget()->DesiredSize
	return	SNew(SBox)
...
void SScreenLog::MakeSelectCategoriesSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection("OutputLogCategoriesEntries");
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SScreenLog::CategoriesShowAll_Execute),
				FCanExecuteAction::CreateLambda([] { return true; }),
				FIsActionChecked::CreateSP(this, &SScreenLog::CategoriesShowAll_IsChecked)),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
		FTextBlockStyle filterTextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText");
		...
		for (const FName Category : Filter.GetAvailableLogCategories())
		{
			TSharedRef<SWidget> messagesButton = SNew(STextBlock)
				.TextStyle(&filterTextStyle)
				.Text(FText::AsCultureInvariant(Category.ToString()))
				.ToolTipText(FText::Format(LOCTEXT("Category_Tooltip", "Filter the Output Log to show category: {0}"), FText::AsCultureInvariant(Category.ToString())));

			MenuBuilder.AddMenuEntry(
				FUIAction(FExecuteAction::CreateSP(this, &SScreenLog::CategoriesSingle_Execute, Category),
					FCanExecuteAction::CreateLambda([] { return true; }),
					FIsActionChecked::CreateSP(this, &SScreenLog::CategoriesSingle_IsChecked, Category)),
				messagesButton,
				NAME_None,
...
bool SScreenLog::VerbosityLogs_IsChecked() const
{
	return Filter.bShowLogs;
}

bool SScreenLog::VerbosityWarnings_IsChecked() const
{
	return Filter.bShowWarnings;
}

bool SScreenLog::VerbosityErrors_IsChecked() const
{
	return Filter.bShowErrors;
}

void SScreenLog::VerbosityLogs_Execute()
{
...
void SScreenLog::VerbosityWarnings_Execute()
{
...
void SScreenLog::VerbosityErrors_Execute()
{
...
bool SScreenLog::CategoriesShowAll_IsChecked() const
{
	return Filter.bShowAllCategories;
}

bool SScreenLog::CategoriesSingle_IsChecked(FName InName) const
{
	return Filter.IsLogCategoryEnabled(InName);
}

void SScreenLog::CategoriesShowAll_Execute()
{
...
void SScreenLog::CategoriesSingle_Execute(FName InName)
{
...
bool FGameLogFilter::IsMessageAllowed(const TSharedPtr<FGameLogMessage>& Message)
{
	// Filter Verbosity
	{
	...
	{
		if (!TextFilterExpressionEvaluator.TestTextFilter(FGameLogFilter_TextFilterExpressionContext(*Message)))
		{
			return false;
		}
	}
...
void FGameLogFilter::AddAvailableLogCategory(FName& LogCategory)
{
...
void FGameLogFilter::ToggleLogCategory(const FName& LogCategory)
{
...
bool FGameLogFilter::IsLogCategoryEnabled(const FName& LogCategory) const
{
	return SelectedLogCategories.Contains(LogCategory);
}

void FGameLogFilter::ClearSelectedLogCategories()
{

...
class FMenuBuilder;
class FScreenLogTextLayoutMarshaller;
class FTextLayout;
class SMenuAnchor;
...
struct FGameLogMessage
{
	...
	FGameLogMessage(const TSharedRef<FString>& NewMessage, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
	...
	FGameLogMessage(const TSharedRef<FString>& NewMessage, ELogVerbosity::Type NewVerbosity, FName NewCategory, FName NewStyle = NAME_None)
		: Message(NewMessage)
...
public:
	...
	SLATE_BEGIN_ARGS(SGameConsoleInputBox)
...
private:
	...
	FEditableTextBoxStyle ETBStyle;
	FSlateBoxBrush ConsoleBorderBrus;
};

/**
* Holds information about filters
*/
struct FGameLogFilter
{
	/** true to show Logs. */
	bool bShowLogs;
	...
	FGameLogFilter() : TextFilterExpressionEvaluator(ETextFilterExpressionEvaluatorMode::BasicString)
	{
	...
	bool IsMessageAllowed(const TSharedPtr<FGameLogMessage>& Message);
...
public:
	...
	SLATE_BEGIN_ARGS(SScreenLog)
		: _Messages()
	{}

	/** All messages captured before this log window has been created */
	SLATE_ARGUMENT(TArray< TSharedPtr<FGameLogMessage> >, Messages)
	...
	*/
	...
	/**
	* Creates FGameLogMessage objects from FOutputDevice log callback
	*
	* @param	V Message text
	* @param Verbosity Message verbosity
	* @param Category Message category
	* @param OutMessages Array to receive created FGameLogMessage messages
	* @param Filters [Optional] Filters to apply to Messages
	*
	* @return true if any messages have been created, false otherwise
	*/
	static bool CreateLogMessages(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category, TArray< TSharedPtr<FGameLogMessage> >& OutMessages);
...
protected:
	...
	*/
	...
	TSharedPtr< FScreenLogTextLayoutMarshaller > MessagesTextMarshaller;
...
public:
	...
	FGameLogFilter Filter;
	FSearchBoxStyle SearchStyle;
	FEditableTextBoxStyle ETBStyle;
};

/** Output log text marshaller to convert an array of FGameLogMessages into styled lines to be consumed by an FTextLayout */
class FScreenLogTextLayoutMarshaller : public FBaseTextLayoutMarshaller
{
public:

	static TSharedRef< FScreenLogTextLayoutMarshaller > Create(TArray< TSharedPtr<FGameLogMessage> > InMessages, FGameLogFilter* InFilter);

	virtual ~FScreenLogTextLayoutMarshaller();
...
protected:
	...
	FScreenLogTextLayoutMarshaller(TArray< TSharedPtr<FGameLogMessage> > InMessages, FGameLogFilter* InFilter);

	void AppendMessageToTextLayout(const TSharedPtr<FGameLogMessage>& InMessage);
	void AppendMessagesToTextLayout(const TArray<TSharedPtr<FGameLogMessage>>& InMessages);

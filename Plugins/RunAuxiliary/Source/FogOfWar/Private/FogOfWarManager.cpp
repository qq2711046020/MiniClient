...
AFogOfWarManager::AFogOfWarManager()
	: SightRange(10.f)
	, SamplesPerMeter(4.f)
	, UnfogColor(uint8(255))
	, FowMaskColor(uint8(200))
	//, bInverseTransformInStreaming(true)
{
...
void AFogOfWarManager::RegisterStaticUnfogged(FVector InLocation, float InRadius)
{
	FScopeLock Lock(&CriticalSection);
	StaticModifier.Add(FFOWStaticModifier_UnFogged(InLocation, InRadius));
}

void AFogOfWarManager::GetStaticUnfogged(TArray<FFOWStaticModifier_UnFogged>& OutUnFogged)
{
	FScopeLock Lock(&CriticalSection);
	OutUnFogged = StaticModifier;
}
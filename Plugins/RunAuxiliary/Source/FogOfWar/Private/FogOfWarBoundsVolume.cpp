...
AFogOfWarBoundsVolume::AFogOfWarBoundsVolume(const FObjectInitializer& ObjectInitializer)
...
void AFogOfWarBoundsVolume::OnConstruction(const FTransform& Transform)
{
...
void AFogOfWarBoundsVolume::PostInitializeComponents()
{
...
void AFogOfWarBoundsVolume::Destroyed()
{
	Super::Destroyed();
}

bool AFogOfWarBoundsVolume::ShouldTickIfViewportsOnly() const
{
	return true;
}

void AFogOfWarBoundsVolume::Tick(float DeltaTime)
{
...
#if WITH_EDITOR
void AFogOfWarBoundsVolume::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	UpdateVolumeTransform();
...
void AFogOfWarBoundsVolume::UpdateVolumeTransform()
{
...
FVector AFogOfWarBoundsVolume::GetCellExtent() const
{
	return CachedCellExtent;
}

FIntPoint AFogOfWarBoundsVolume::TransformWorldToLayer(const FVector& AgentLocation) const
{
...
int32 AFogOfWarBoundsVolume::ScaleDistanceToLayer(const int32 Distance) const
{
	// For now we assume that volume is square and use X only
	return Distance / VolumeTransform.GetScale3D().X;
}

void AFogOfWarBoundsVolume::DrawDebugGrid()
{
...
FVector AFogOfWarBoundsVolume::SnapWorldToGrid(const FVector& InLocation) const
{
